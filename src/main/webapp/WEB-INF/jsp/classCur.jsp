...
<body>
				...
				<div class="row-fluid">
					<div class="box span12">
						<div class="box-header" data-original-title="">
							<h2>
								<i class="icon-user-md"></i><span class="break"></span>班级课程表
							</h2>
							<div class="box-icon">
						...
						<div class="box-content">
							<table
								...
								<tbody>
									<c:forEach items="${claCurList }" var="claCurOne">
										<tr>
											<td>${claCurOne.cur_teacher_id }</td>
											<td class="center">${claCurOne.cur_name }</td>
											<td class="center">${claCurOne.teacher.teacher_name }</td>
											<td class="center">${claCurOne.teacher.teacher_phone }</td>
											<td class="center"><a class="btn btn-danger" href="#"
												data-toggle="modal"
												data-target="#deleteClaCurOne${claCurOne.cur_teacher_id}"
												title="删除"> <i class="icon-trash "></i>
											</a>
												<div class="modal fade" tabindex="-1" role="dialog"
													id="deleteClaCurOne${claCurOne.cur_teacher_id}"
													style="display: none">
													>
														...
														<div class="modal-content">
															...
															<div class="modal-footer">
																...
																<button type="button" class="btn btn-primary"
																	id="delete_claCur_but1"
																	onClick="delete_claCur(${claCurOne.cur_teacher_id},${claId })">确认</button>
															</div>
														</div>
													</div>
												</div></td>
							...
							<table
								...
								<tbody>
									<c:forEach items="${curTeaList }" var="curTeaOne">
										<tr>
											<td>${curTeaOne.cur_teacher_id }</td>
											<td class="center">${curTeaOne.cur_name }</td>
											<td class="center">${curTeaOne.teacher.teacher_name }</td>
											<td class="center">${curTeaOne.teacher.teacher_phone }</td>
											<td class="center"><a class="btn btn-info" href="#"
												data-toggle="modal"
												data-target="#addOneClaCur${curTeaOne.cur_teacher_id }"
												title="添加教师"> <i class=" icon-ok "></i>
											</a>
												<div class="modal fade" tabindex="-1" role="dialog"
													id="addOneClaCur${curTeaOne.cur_teacher_id }" tabindex="-1"
													style="display: none">
													<div class="modal-dialog">
														<div class="modal-content">
															<div class="modal-header">
															...
															<div class="modal-footer">
																...
																<button type="button" class="btn btn-primary"
																	id="add_claCur_but1"
																	onClick="add_claCur(${curTeaOne.cur_teacher_id },${claId })">确认</button>
															</div>
														</div>
													</div>
												</div></td>
	...
	<!-- end: JavaScript-->
	<script>
	function delete_claCur(curTeaId,claId)
	{
 		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/classroom/course",
	...
	function add_claCur(curTeaId,claId)
	{
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/classroom/course",
